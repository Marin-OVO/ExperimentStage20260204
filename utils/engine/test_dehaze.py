"""
    ...
    2026-03-09 experiment: confidence
"""
import time
...
def train_one_epoch(
    ...
    for step, (images, targets) in enumerate(train_dataloader):
        ...
        with ((autocast())):
            ...
            # density_out = outputs['density_out'].to('cpu').detach().numpy()[0]
            # if step == 0:
            #     vis_density_map = density_out.squeeze(0)
            #
            #     plt.figure()
            #     plt.imshow(vis_density_map, cmap="jet")
            #     plt.show()
