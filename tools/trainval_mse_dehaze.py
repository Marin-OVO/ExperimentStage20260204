...
from model import UNetTestAttDehaze
from datasets import CrowdDataset
from utils import custom_collate_fn
from utils.metrics import PointsMetrics
from utils.engine.test_dehaze import train_one_epoch, val_one_epoch
...
def main(args):
    ...
    model = UNetTestAttDehaze(in_channels=3, num_class=args.num_classes, bilinear=args.bilinear)
    model.to(device)
    ...
    for epoch in range(last_epoch, args.epoch): # (0 - 149)
        logger.info('=' * 60)
        logger.info('Epoch [{:03d}/{:03d}]'.format(epoch + 1, args.epoch))
    ...
    if len(map_list) != 0: # val
        ...
        plot_map(map_list, min=1, save_dir=work_dir)

    if len(f1_list) != 0: # val
        from utils.plot_curve import plot_f1
        plot_f1(f1_list, min=1, save_dir=work_dir)


if __name__ == "__main__":
    from configs.train_mse_dehaze import args_parser
    args = args_parser()
