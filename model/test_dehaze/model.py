...
class UNetTestAttDehaze(nn.Module):
    def __init__(self, in_channels, num_class=2, bilinear=False):
        super(UNetTestAttDehaze, self).__init__()
        ...
        self.predictor_s = Predictor(in_channels=64, out_channels=1)
        self.predictor_block = PredictorPBlock(in_channels=64, out_channels=64)

        self.dehaze_block = DehazeBlock(in_channels=64, out_channels=64)
    def forward(self, Ci):
        # Down
        # Ci_tx = self.dehaze(Ci)
        # Ci = (Ci - self.A) / (Ci_tx + 1e-6) + self.A
        ...
        # heatmap_out = self.heatmap_head(x2_out)
        # ====== predictor block ======
