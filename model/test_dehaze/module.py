...
class DehazeNet(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        self.feature_extraction = nn.Conv2d(in_channels, 16, kernel_size=5, padding=2)
        ...
        self.regression = nn.Conv2d(48, 1, kernel_size=7, padding=3)
    def forward(self, x):
        ...
        x = F.max_pool2d(x, kernel_size=7, stride=1, padding=3)
        x = torch.clamp(self.regression(x), 0.1, 1.0)
